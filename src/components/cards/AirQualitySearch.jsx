...
const AirQualitySearch = () => {
  ...
  return (
    <>
      ...
      {
        (cityLoading && searchCity.length > 2) ||
        (airLoading && cityData?.[0]?.lat) ? (
        ...
        ) : searchCity.length > 0 && !cityData.length ? (
          <div className="card-air"><p className="no-data">No AQI data found for {searchCity}</p></div>
